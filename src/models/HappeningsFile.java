        ...
        for (int i = 1; i < 7; i++) {
            for (int j = 1; j < 7; j++) {
                for (int k = 0; k < ArrayLists.grassList.size(); k++) {
                    if (ArrayLists.grassList.get(k).isAvailable && ArrayLists.grassList.get(k).x == j && ArrayLists.grassList.get(k).y == i) {
                        grass[i-1][j-1]++;
                    }
                }
            }
...
//        fileCleaner.close();
        ...
        int[] tasks=MissionFileProcessor.task();
        for (int i = 0; i < tasks.length; i+=2) {
            fileWriter.write(tasks[i+1]+"/"+tasks[i]+"\n");
        }
        fileWriter.close();
